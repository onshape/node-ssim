...
test('Expect differences object does not have extra properties', async () => {
  ...
  const differences = await ssim.compare(OnshapeImagePath, OnshapeImagePath, compareOpts);
  expect(Object.keys(differences).sort()).toEqual(expectedKeys.sort());
});

test('Compare image to itself', async () => {
  const differences = await ssim.compare(OnshapeImagePath, OnshapeImagePath, compareOpts);
...
test('Compare image to inverted color version of itself', async () => {
  const differences = await ssim.compare(OnshapeImagePath, OnshapeInvertedColorsImagePath, compareOpts);
...
test('Compare image to flipped version of itself', async () => {
  const differences = await ssim.compare(OnshapeImagePath, OnshapeUpsideDownImagePath, compareOpts);
